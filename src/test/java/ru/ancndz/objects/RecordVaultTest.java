...
public class RecordVaultTest {
    ...
    public void testAddRecord() {
        ...
        Assertions.assertEquals(testRecord1, this.recordVault.getHigherTrafficRecord());

        this.recordVault.addRecord(testRecord2);
        // Добавляем первую запись, лидер изменился
        Assertions.assertEquals(testRecord2, this.recordVault.getHigherTrafficRecord());
        // Сравним рейтинги:
        Assertions.assertTrue(this.recordVault.getHigherTrafficRecord().getTrafficRating() >
                testRecord1.getTrafficRating());

        this.recordVault.addRecord(testRecord3);
        // Добавляем следующую запись, лидер не изменился
        Assertions.assertNotEquals(testRecord3, this.recordVault.getHigherTrafficRecord());
        // Сравним рейтинги:
        Assertions.assertTrue(this.recordVault.getHigherTrafficRecord().getTrafficRating() >
        ...
        Assertions.assertEquals(testRecord4, this.recordVault.getHigherTrafficRecord());
        // Сравним рейтинги:
        Assertions.assertEquals(testRecord2.getTrafficRating(), this.recordVault.getHigherTrafficRecord().getTrafficRating());
        Assertions.assertTrue(this.recordVault.getHigherTrafficRecord().getTrafficAccidents() > testRecord2.getTrafficAccidents());
    }
